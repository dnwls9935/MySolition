...
HRESULT HyperionChest::SettingItem()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);

	m_ItemPositionBone = m_pModelCom->Get_BoneMatrix("PanelBase");
	if (nullptr == m_ItemPositionBone)
		return E_FAIL;

	PickUps::PUDESC		PUDesc;
	ZeroMemory(&PUDesc, sizeof(PickUps::PUDESC));
	PUDesc.m_TypeID = PickUps::TYPE_ID::HEALTH_VIRAL;
...
_matrix HyperionChest::GetBoneMatrix()
{
	_matrix TransformMatrix = XMMatrixTranslation(0.f, 0.f, -5000.f);
