#include "..\public\VIBuffer_PointInstance_CollisionDust.h"

#include "Calculator.h"

CVIBuffer_PointInstance_CollisionDust::CVIBuffer_PointInstance_CollisionDust(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CVIBuffer(pDevice, pDeviceContext)
{

}

CVIBuffer_PointInstance_CollisionDust::CVIBuffer_PointInstance_CollisionDust(const CVIBuffer_PointInstance_CollisionDust & rhs)
	: CVIBuffer(rhs)
...
HRESULT CVIBuffer_PointInstance_CollisionDust::NativeConstruct_Prototype(const _tchar* pShaderFilePath, _uint _NumInstance)
{
...
HRESULT CVIBuffer_PointInstance_CollisionDust::NativeConstruct(void * pArg)
{
	...
	m_Speed = new _float[m_NumInstance];
	for (_uint i = 0; i < m_NumInstance; i++) {
		m_Speed[i] = rand() % 20 + 5;
	}

	m_B = new _float[m_NumInstance];
	for (_uint i = 0; i < m_NumInstance; i++) {
		m_B[i] = 0.f;
	}
...
HRESULT CVIBuffer_PointInstance_CollisionDust::Render(_uint _PassIndex)
{
...
_bool CVIBuffer_PointInstance_CollisionDust::Update(_double _TimeDelta)
{
	...
	for (_uint i = 0; i < m_NumInstance; i++)
	{
		//m_B[i] = Calculator::Lerp(m_B[i], m_H[i], _TimeDelta*  m_Speed[i], Lerp::EaseType::EaseOutSine);
		((VTXMATRIX*)SubResource.pData)[i].vPosition.y -= _TimeDelta*  m_Speed[i];
...
CVIBuffer_PointInstance_CollisionDust * CVIBuffer_PointInstance_CollisionDust::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, const _tchar* pShaderFilePath, _uint _NumInstance)
{
	CVIBuffer_PointInstance_CollisionDust*		pInstance = new CVIBuffer_PointInstance_CollisionDust(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype(pShaderFilePath, _NumInstance)))
	{
		MSGBOX("Failed to Creating CVIBuffer_PointInstance_CollisionDust");
...
CComponent * CVIBuffer_PointInstance_CollisionDust::Clone(void * pArg)
{
	CVIBuffer_PointInstance_CollisionDust*		pInstance = new CVIBuffer_PointInstance_CollisionDust(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSGBOX("Failed to Cloning CVIBuffer_PointInstance_CollisionDust");
		Safe_Release(pInstance);
	}
...
void CVIBuffer_PointInstance_CollisionDust::Free()
{
	__super::Free();
